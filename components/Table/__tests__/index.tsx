...
const getWrapperShallow = (props: any) => shallow(<Table {...props} />);
...
describe("Table", () => {
  ...
  it("sets the caption of the table when cation prop is passed", () => {
    ...
    expect(wrapper.find("Table__StyledCaption").text()).toBe(
      "tipresias_2020 predictions for matches of round 1, season 2020"
    );
  });
  ...
  it("sets the column headings when the headers prop is passed", () => {
    ...
    expect(wrapper.find("Table__StyledTableHeading").first().text()).toBe(
      "Date"
    );
    expect(wrapper.find("Table__StyledTableHeading").last().text()).toBe(
      "is Correct?"
    );
  });
