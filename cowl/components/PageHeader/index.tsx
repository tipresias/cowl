...
const ListItem = styled.li`
  ...
  @media (max-width: 425px) {
    display: ${(props: { displayInMobile: boolean }) => (props.displayInMobile ? 'block' : 'none')};
  }
`;
...
const PageHeader = ({ links, isDarkMode, toggleDarkMode }: Props) => (
  <Header>
    <Link href="/">
      <Logo><Image src={logo} alt="Tipresias" width={120} height={60}></Image></Logo>
    </Link>
    <nav>
      <ListStyled>
        {
          links.map(
            link => (
              <ListItem key={link.url} displayInMobile={false}>
                <TextLink><Link href={link.url}>{link.text}</Link></TextLink>
              </ListItem>
            ),
          )
        }
