...
describe("Checkbox", () => {
  ...
  it("label is associated to the checkbox", () => {
    ...
    expect(wrapper.find('input[id="Checkbox_id"]').prop("id")).toBe(
      "Checkbox_id"
    );
    expect(wrapper.find('label[htmlFor="Checkbox_id"]').prop("htmlFor")).toBe(
      "Checkbox_id"
    );
  });
  ...
  it("when checked calls onChange", () => {
    ...
    wrapper
      .find('input[id="Checkbox_id"]')
      .simulate("change", eventObjectMocked);
    expect(onChangeSpy).toHaveBeenCalledWith({
      target: { value: "Checkbox_name123" },
    });
  });
});
