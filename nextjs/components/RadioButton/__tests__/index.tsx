...
describe("RadioButton", () => {
  ...
  it("label is associated to the RadioButton", () => {
    ...
    expect(wrapper.find('input[id="RadioButton_id"]').prop("id")).toBe(
      "RadioButton_id"
    );
    expect(
      wrapper.find('label[htmlFor="RadioButton_id"]').prop("htmlFor")
    ).toBe("RadioButton_id");
  });
  ...
  it("when checked calls onChange", () => {
    ...
    const wrapper = shallow(
      <RadioButton
        ...
        value="RadioButton_name"
        onChange={onChangeSpy}
      />
    );
    wrapper
      .find('input[id="RadioButton_id"]')
      .simulate("change", eventObjectMocked);
    expect(onChangeSpy).toHaveBeenCalledWith({
      target: { value: "RadioButton_name123" },
    });
  });
});
